...
.NOTES
    Author:  Ernesto Cobos Roqueñí (@ecobos)
    Date:    2026-02-26
    Version: 3.8
...
# Validate that the domain resolves at all
$dnsCheck = Resolve-DnsName -Name $domain -Type ANY -ErrorAction SilentlyContinue
if ($null -eq $dnsCheck) {
    Write-Host ""
    Write-Host "================================================================" -ForegroundColor Red
    Write-Host "  ERROR: El dominio '$domain' no devuelve informacion DNS."       -ForegroundColor Red
    Write-Host "  Posibles causas:"                                               -ForegroundColor Yellow
    Write-Host "    - El dominio no existe o esta mal escrito"                    -ForegroundColor Yellow
    Write-Host "    - El servidor DNS no responde"                                -ForegroundColor Yellow
    Write-Host "    - El dominio no tiene registros DNS publicos"                 -ForegroundColor Yellow
    Write-Host "================================================================" -ForegroundColor Red
    Write-Host ""
    exit 1
}
...
<head>
    ...
    <style>
        ...
        .link-Portal { color: #0078d4; text-decoration: none; font-weight: 600; }
        .link-Portal:hover { text-decoration: underline; color: #005a9e; }
...
$reportPath = "C:\Users\ecobos\OneDrive - Microsoft\ecobos\Documents\05 Scripts\Sender Authentication\$Filehtml"
